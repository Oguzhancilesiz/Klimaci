...
namespace Klimaci.WebUI.Areas.Admin.Controllers
{
    ...
    {
        private readonly IPartnerService _svc; private readonly IUnitOfWork _uow; private readonly IWebHostEnvironment _env;
        public PartnerController(IPartnerService svc, IUnitOfWork uow, IWebHostEnvironment env) { _svc = svc; _uow = uow; _env = env; }

        [HttpGet] public async Task<IActionResult> Index([FromQuery] ListQueryDTO q) => View(await _svc.PagedAsync(q ?? new()));
        [HttpGet] public IActionResult Create() => View(new PartnerAddDTO("", null, true, null, null, null, 0, true, null, null));
        [HttpPost]
        [ValidateAntiForgeryToken]
        public async Task<IActionResult> Create(PartnerAddDTO dto, List<IFormFile> files)
        { var ids = await SaveMediasAsync(files, "uploads/media"); dto = dto with { MediaIds = (dto.MediaIds ?? new()).Concat(ids).Distinct().ToList() }; await _svc.Add(dto); TempData["ok"] = "Partner eklendi."; return RedirectToAction(nameof(Index)); }

        [HttpGet]
        public async Task<IActionResult> Edit(Guid id)
        {
            var e = await _svc.GetById(id); if (e == null) return NotFound(); ViewBag.Gallery = await _svc.GetGalleryAsync(id);
            var dto = new PartnerUpdateDTO(e.Id, e.Title, e.Website, e.IsFeatured, e.Slug, e.SeoTitle, e.SeoDescription, e.DisplayOrder, e.IsPublished, null, null); return View(dto);
        }

        [HttpPost]
        [ValidateAntiForgeryToken]
        public async Task<IActionResult> Edit(PartnerUpdateDTO dto, List<IFormFile> newFiles)
