...
namespace Klimaci.WebUI.Areas.Admin.Controllers
{
    ...
    {
        private readonly IBlogService _svc; private readonly IUnitOfWork _uow; private readonly IWebHostEnvironment _env;
        public BlogPostController(IBlogService svc, IUnitOfWork uow, IWebHostEnvironment env) { _svc = svc; _uow = uow; _env = env; }

        [HttpGet] public async Task<IActionResult> Index([FromQuery] BlogListQueryDTO q) => View(await _svc.PagedAsync(q ?? new()));

        [HttpGet]
        public async Task<IActionResult> Create()
        {
            ViewBag.Categories = await _uow.Repository<Category>().GetAll();
            ViewBag.Tags = await _uow.Repository<Tag>().GetAll();
            return View(new BlogPostAddDTO("", null, "", null, new(), new(), null, null, null, 0, true, null, null));
        }

        [HttpPost]
        [ValidateAntiForgeryToken]
        public async Task<IActionResult> Create(BlogPostAddDTO dto, List<IFormFile> files)
        {
            var ids = await SaveMediasAsync(files, "uploads/media");
            dto = dto with { MediaIds = (dto.MediaIds ?? new()).Concat(ids).Distinct().ToList() };
            await _svc.Add(dto); TempData["ok"] = "Yazı eklendi."; return RedirectToAction(nameof(Index));
        }
        ...
        public async Task<IActionResult> Edit(Guid id)
        {
            ...
            var dto = new BlogPostUpdateDTO(e.Id, e.Title, e.Summary, e.Content, e.PublishDate, null, null, e.Slug, e.SeoTitle, e.SeoDescription, e.DisplayOrder, e.IsPublished, null, null);
            return View(dto);
        }

        [HttpPost]
        [ValidateAntiForgeryToken]
        public async Task<IActionResult> Edit(BlogPostUpdateDTO dto, List<IFormFile> newFiles)
        {
            var ids = await SaveMediasAsync(newFiles, "uploads/media");
            dto = dto with { MediaIds = (dto.MediaIds ?? new()).Concat(ids).Distinct().ToList() };
            await _svc.Update(dto); TempData["ok"] = "Güncellendi."; return RedirectToAction(nameof(Index));
        }
